<template>
	<div class="cart-wrapper">
						...
						<li v-for="item in cartList">
							<div class="cart-tab-1">
								<div class="cart-item-check">
									<a href="javascript:;" :class="{'check':item.checked=='1'}" class="checkbox-btn item-check-btn" @click="editCart('checked',item)">
							...
							<div class="cart-tab-3">
								<div class="item-quantity">
									<div class="select-self select-self-open">
										<div class="select-sel-area">
											<a href="javascript:;" class="input-sub" @click="editCart('minu',item)">-</a>
											<span class="select-ipt">{{item.productNum}}</span>
											<a href="javascript:;" class="input-sub" @click="editCart('add',item)">+</a>
										</div>
									</div>
								</div>
							</div>
				...
				<div class="cart-foot-inner">
					...
					<div class="cart-foot-r">
						<div class="item-total">
							总计(total): <span>{{totalPrice|currency('$')}}</span>
						</div>
						<div class="btn-wrap">
							<a class="btn btn--red" :class="{'btn--dis':checkedCount==0}" @click="checkOut">结账(checkout)</a>
						</div>
					</div>
				</div>
			</div>
		...
		<modal :mdShow="modalConfirm" @close="closeModal">
			...
			<div slot="btnGroup">
				<a @click="delCart" href="javascript:;" class="btn btn--m">确认</a>
				<a @click="closeModal" href="javascript:;" class="btn btn--m btn--red">关闭</a>
			</div>
		</modal>
...
<script type="text/ecmascript-6">
	...
	export default{
		...
		methods: {
			...
			delCartConfirm(item) {
				this.delItem = item;  //先拿到这个对象
				this.modalConfirm = true;
			},
			delCart(item) {
				axios.post('/users/cartDel', {
					productId: this.delItem.productId //这样才能把对象带过去
				}).then((response) => {
					let res = response.data;
					if(res.status === '0'){
						this.modalConfirm = false;
			...
			editCart(flag, item) {
				...
				axios.post('/users/cartEdit', {  //请求服务器  把数据保存到数据库中去 
